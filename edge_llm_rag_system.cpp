...
namespace edge_llm_rag
{
    ...
    void EdgeLLMRAGSystem::rag_message_worker(const std::string &rag_text)
    {
        ...
        while (it != end)
        {
            ...
            {
                std::cout << "[TTS] " << converter.to_bytes(wide_segment) << std::endl;
            }
            ++it;
        }
        ...
        if (last_pos < wide_text.length())
        {
            ...
            {
                std::cout << "[TTS] " << converter.to_bytes(last_segment) << std::endl;
            }
        }
    }
    ...
    {
        auto response = llm_callback(query);
    ...
    {
        auto response = llm_callback(prompt);
