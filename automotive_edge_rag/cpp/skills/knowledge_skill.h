...
namespace edge_llm_rag
{
    ...
    class KnowledgeSkill : public BaseSkill
    {
    public:
        explicit KnowledgeSkill(const std::string &name,
                                const std::string &description,
                                const std::string &domain,
                                const std::vector<std::string> &keywords);

        ~KnowledgeSkill() override = default;

        SkillResult execute(const std::string &query,
